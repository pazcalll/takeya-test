<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <a href="" class="inline-flex gap-2 items-center text-blue-500" id="redirect-back">
    ...
    <script>
        let redirectBackUrl = null;

        if (document.referrer != '') {
            if (document.referrer == "{{ route('posts.show', $post->id) }}") redirectBackUrl = "{{ route('home') }}";
            else redirectBackUrl = document.referrer;
        }

        else redirectBackUrl = "{{ route('home') }}";

        document.getElementById('redirect-back').setAttribute('href', redirectBackUrl)
    </script>
</x-app-layout>
